...
function buildP(){
  ...
  const zadaniInstr=`Na úplný začátek odpovědi přidej jeden řádek: '📖 Zadání: [jen klíčová čísla a operace které vidíš, max 60 znaků]' Pak pokračuj s výsledky.`;
  if(opts.form==='STEP_BY_STEP'){return `Analyzuj tento test a pro každou matematickou nebo fyzikální úlohu vytvoř postup krok za krokem. Jazyk: ${opts.lang}. ${opts.len} Každá úloha: název <h3>, zadáno <div class="highlight">Zadáno: ...</div>, kroky <div class="step"><span class="stepnum">Krok N</span> popis</div>, výsledek <div class="highlight">✓ Výsledek: ...</div>. Ostatní otázky stručně <ul><li>. ${diagramRules} ${zadaniInstr}${ex?'\n'+ex:''}`}
  return `Vytvoř výsledky z tohoto testu. Jazyk: ${opts.lang}. ${opts.len} ${opts.form} Pro každou otázku PŘÍMÁ ODPOVĚĎ. HTML: <h2> nadpis, <h3> sekce, <ul><li>, <strong>, <div class="highlight"> vzorce. ${diagramRules} ${zadaniInstr}${ex?'\n'+ex:''}`;
}
